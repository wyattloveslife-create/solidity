...
/// Pseudo-randomly generated 64-byte fallback calldata used when no calldata
/// is present in the proto (e.g., old corpus entries pre-dating the calldata field).
static bytes const s_defaultCalldata = {
...
void ProtoConverter::visit(Program const& _x)
{
	...
	// Create Calldata; fall back to fixed default for old corpus entries without the field.
	m_calldata = _x.has_calldata() ? createCalldata(_x.calldata()) : s_defaultCalldata;
